...
```{r load-kallcounts}
kallcountfile <- here::here("data/misc","CW_kallisto1_abundance_long_by_sample_by_isoform.tsv")
...
kallcounts_bygene <- 
  ...
  dplyr::summarise(est_counts=sum(est_counts),TPM = sum(tpm))
...
```{r write-kallcounts-by-gene-fc}
kallcounts_bygene_fcfile <-
    here::here("data","CW_kallisto_abundance_fold_change_long_by_gene.tsv")

write_lines(c("# CW_kallisto_abundance_fold_change_long_by_gene.tsv",
              ...
              "# Contents:",
              "#   code: Short sample description, see crypto_wake_up_sample_sheet.txt",
              "#   gene: C. neoformans H99 systematic ORF name",
              "#   est_counts: estimated gene counts from kallisto",              
              "#   TPM: estimated transcripts per million from kallisto",  
              "#   fold_change: TPM relative to median for one gene across all samples",
              "# "),
...
```{r make-fold-change-wide}
kallcounts_bygene_fc_wide <- 
    kallcounts_bygene_fc %>%
    tidyr::pivot_wider(id_cols="gene", names_from = "code", values_from = "fold_change")
...
```{r make-explicit-metadata}
sample_metadatafile <- here::here("data","crypto_wake_up_sample_sheet.tsv")
...
```{r write-kallcounts-by-gene-mean-by-condition, eval=FALSE, include=FALSE}
